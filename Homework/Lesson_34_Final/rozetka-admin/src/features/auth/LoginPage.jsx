import { Form, Field } from 'react-final-form';
import {
    TextField,
    Button,
    Box,
    Typography,
    IconButton,
    InputAdornment,
    Paper
} from '@mui/material';

import { Visibility, VisibilityOff } from '@mui/icons-material';
import { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { Navigate } from 'react-router-dom';

import { login } from './authSlice';
import { required } from '../../utils/validators';

export default function LoginPage() {
    const dispatch = useDispatch();
    const token = useSelector(s => s.auth.token);
    const error = useSelector(s => s.auth.error);

    const [show, setShow] = useState(false);

    if (token) return <Navigate to="/" />;

    const onSubmit = values => {
        dispatch(login(values));
    };

    return (
        <Box
            sx={{
                minHeight: '100vh',
                backgroundColor: '#49A36D',
                display: 'flex',
                justifyContent: 'center',
                alignItems: 'center'
            }}
        >
            <Paper
                elevation={6}
                sx={{
                    width: 420,
                    p: 5,
                    borderRadius: 2,
                    textAlign: 'center'
                }}
            >
                {/* LOGO */}
                <Box sx={{ mb: 4 }}>
                    <svg width="240" height="40" viewBox="0 0 240 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0_3_5)">
                            <path d="M41.0255 20.0882C40.9803 31.1355 31.7406 40.0439 20.4225 39.9998C9.10435 39.9557 -0.0450154 30.9591 0.000166623 19.9118C0.0453487 8.86448 9.28508 -0.043938 20.6032 0.000163037C31.9439 0.0442641 41.0707 9.04088 41.0255 20.0882Z" fill="#221F1F"/>
                            <path d="M27.9005 29.9007C25.7317 33.6934 23.0434 36.8025 20.0388 36.7364C18.1637 36.6922 16.379 36.0087 14.7525 34.9282C15.2721 34.0021 18.5478 29.3715 27.9005 29.9007ZM40.4837 20.0882C40.4385 30.8268 31.4473 39.5147 20.4228 39.4486C9.39841 39.4045 0.542725 30.6504 0.542725 19.9118C0.542725 12.9438 4.11211 7.93834 7.50076 5.24818C11.6575 1.96265 14.4814 4.47641 15.4754 6.39481C15.3624 5.57894 15.0236 3.24158 14.0973 1.56574C16.1305 0.882172 18.3219 0.507313 20.6036 0.529363C31.628 0.595515 40.5288 9.32753 40.4837 20.0882ZM19.4514 8.88652C22.3431 6.96812 27.3357 5.90969 29.9788 9.92289C28.3749 0.749868 21.4394 6.01995 19.4514 8.88652ZM15.4302 17.5524C16.0402 16.8909 16.7405 16.2293 17.5538 15.634C21.1683 12.9218 25.1218 12.2382 26.4095 14.1345C26.9065 14.8622 26.929 15.8765 26.545 16.9791C27.8327 14.9063 28.2393 12.9879 27.4486 11.9295C26.1158 10.1654 21.9816 11.3782 18.2089 14.6417C17.1019 15.5458 16.1757 16.5601 15.4302 17.5524ZM9.12732 19.1841C10.9572 19.5149 12.9452 17.2437 13.5552 14.0684C14.1651 10.8931 13.1937 8.07065 11.3638 7.71784C9.53395 7.38708 7.54594 9.65829 6.93599 12.8336C6.32603 15.9868 7.29744 18.8313 9.12732 19.1841ZM34.5648 15.3473C17.0568 26.7916 4.97057 20.9923 4.97057 20.9923C4.78984 24.9614 11.7705 37.1553 20.084 37.3097C28.4201 37.464 34.5648 15.3473 34.5648 15.3473ZM7.43299 12.8997C7.99777 9.98904 9.71468 7.87219 11.2735 8.1809C12.8322 8.46756 13.6455 11.0695 13.1033 13.9802C12.5386 16.8909 10.8216 19.0077 9.26286 18.699C7.68149 18.4124 6.86821 15.8104 7.43299 12.8997ZM10.7539 15.3694C11.4542 15.5237 12.2223 14.6858 12.4934 13.5171C12.7645 12.3485 12.403 11.268 11.7027 11.1357C11.2283 11.0254 10.7313 11.3782 10.3698 11.9736C10.6861 12.1279 10.8442 12.5249 10.6861 12.8556C10.5506 13.1643 10.2343 13.3187 9.918 13.2525C9.78246 14.2889 10.1213 15.215 10.7539 15.3694Z" fill="#05BC52"/>
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M84.8774 20.1102C84.8774 12.9658 90.3505 9.59212 99.0898 9.59212C107.873 9.59212 113.324 12.9879 113.324 20.1102C113.324 27.2325 107.873 30.6504 99.0898 30.6504C90.3284 30.6504 84.8774 27.2546 84.8774 20.1102ZM92.0498 20.0882C92.0498 23.8809 94.7201 25.3803 99.0898 25.3803C103.437 25.3803 106.152 23.9029 106.152 20.0882C106.152 16.2734 103.437 14.8181 99.0898 14.8181C94.7201 14.8181 92.0498 16.2734 92.0498 20.0882Z" fill="#05BC52"/>
                            <path d="M136.784 13.6494V10.0772H114.626V15.215H126.544L114.45 26.571V30.1212H136.85V24.9614H124.822L136.784 13.6494Z" fill="#05BC52"/>
                            <path d="M158.058 22.0948H146.295V25.1598H160.552V30.1212H139.697V10.0772H160.066V15.1268H146.295V18.0816H158.058V22.0948Z" fill="#05BC52"/>
                            <path d="M162.251 15.215H170.571V30.1212H177.413V15.215H185.733V10.0772H162.251V15.215Z" fill="#05BC52"/>
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M230.665 10.0772H221.749L212.392 30.1212H219.851L221.462 26.1962H230.908L232.519 30.1212H240L230.665 10.0772ZM223.117 22.183L226.207 14.6858L229.275 22.183H223.117Z" fill="#05BC52"/>
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M82.8469 17.3319C82.8469 13.0982 80.1324 10.0772 73.6662 10.0772H59.6745V30.1212H66.4276V24.4542H70.7752L75.6303 30.1212H84.0607L78.411 23.6163C81.2359 22.3814 82.8469 19.7795 82.8469 17.3319ZM66.4276 14.7961H73.0704C75.1228 14.7961 75.9393 15.8986 75.9393 17.3098C75.9393 18.258 75.3655 20 72.96 20H66.4276V14.7961Z" fill="#05BC52"/>
                            <path d="M194.913 17.398L203.211 10.0772H211.995L202.35 18.258L211.664 30.1212H203.233L197.451 22.4255L194.913 24.5865V30.1212H188.16V10.0772H194.913V17.398Z" fill="#05BC52"/>
                        </g>
                        <defs>
                            <clipPath id="clip0_3_5">
                                <rect width="240" height="40" fill="white"/>
                            </clipPath>
                        </defs>
                    </svg>
                </Box>

                <Form
                    onSubmit={onSubmit}
                    render={({ handleSubmit }) => (
                        <form onSubmit={handleSubmit}>
                            <Field name="login" validate={required}>
                                {({ input, meta }) => (
                                    <TextField
                                        {...input}
                                        placeholder="User Name"
                                        fullWidth
                                        margin="normal"
                                        variant="filled"
                                        error={meta.touched && meta.error}
                                        helperText={meta.touched && meta.error}
                                        InputProps={{ disableUnderline: true }}
                                        sx={inputStyles}
                                    />
                                )}
                            </Field>

                            <Field name="password" validate={required}>
                                {({ input, meta }) => (
                                    <TextField
                                        {...input}
                                        type={show ? 'text' : 'password'}
                                        placeholder="Password"
                                        fullWidth
                                        margin="normal"
                                        variant="filled"
                                        error={meta.touched && meta.error}
                                        helperText={meta.touched && meta.error}
                                        InputProps={{
                                            disableUnderline: true,
                                            endAdornment: (
                                                <InputAdornment position="end">
                                                    <IconButton onClick={() => setShow(!show)}>
                                                        {show ? <VisibilityOff /> : <Visibility />}
                                                    </IconButton>
                                                </InputAdornment>
                                            )
                                        }}
                                        sx={inputStyles}
                                    />
                                )}
                            </Field>

                            {error && (
                                <Typography color="error" mt={1}>
                                    {error}
                                </Typography>
                            )}

                            <Button
                                type="submit"
                                fullWidth
                                variant="contained"
                                sx={{
                                    mt: 3,
                                    height: 45,
                                    backgroundColor: '#49A36D',
                                    fontSize: 16,
                                    fontWeight: 700,
                                    '&:hover': { backgroundColor: '#3a8a5b' }
                                }}
                            >
                                Login
                            </Button>
                        </form>
                    )}
                />
            </Paper>
        </Box>
    );
}

const inputStyles = {
    '& .MuiFilledInput-root': {
        backgroundColor: '#eee'
    },
    borderRadius: 1,
    '& .MuiInputBase-root': {
        height: 48
    },
    '& .MuiInputBase-input': {
        height: '100%',
        display: 'flex',
        alignItems: 'center',
        padding: '0 14px'
    },
    '& input::placeholder': {
        color: '#00A046',
        opacity: 1
    }
};
